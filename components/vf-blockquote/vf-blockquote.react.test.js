...
describe('VfBlockquote', () => {
    ...
    test('renders with small modifier', async () => {
        await render(<VfBlockquote blockquote_text="Test blockquote" modifier="small" />);
        expect(screen.getByRole('blockquote')).toHaveClass('vf-blockquote-small');
    });

    test('renders with override class', async () => {
        await render(<VfBlockquote blockquote_text="Test blockquote" override_class="custom-class" />);
        expect(screen.getByRole('blockquote')).toHaveClass('custom-class');
    });

    test('renders blockquote author', async () => {
        await render(<VfBlockquote blockquote_text="Test blockquote" blockquote_author="Author Name" />);
        expect(screen.getByText('Author Name')).toBeInTheDocument();
    });

    test('renders blockquote author with href', async () => {
        await render(<VfBlockquote blockquote_text="Test blockquote" blockquote_author="Author Name" blockquote_author_href="https://example.com" />);
    ...
    test('renders blockquote author details', async () => {
        await render(<VfBlockquote blockquote_text="Test blockquote" blockquote_author_details="Author Details" />);
        expect(screen.getByText('Author Details')).toBeInTheDocument();
    });

    test('renders blockquote author image', async () => {
        await render(<VfBlockquote blockquote_text="Test blockquote" blockquote_author_imageurl="https://example.com/image.jpg" />);
        const img = screen.getByAltText('');
        expect(img).toHaveAttribute('src', 'https://example.com/image.jpg');
    });

    test('renders with id attribute', async () => {
        await render(<VfBlockquote blockquote_text="Test blockquote" id="test-id" />);
