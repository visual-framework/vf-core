...
function vfGaInit(vfGaTrackOptions) {
  ...
  if (vfGaTrackOptions.vfGaTrackNetwork != null) {
    ...
    providePlugin('ipMeta', IpMeta); // Track the network
...
function vfSearchClientSide(firstPassedVar) {
  ...
  var customPipelineWithoutHypens = function customPipelineWithoutHypens(builder) {
  ...
  function renderResults() {
    ...
    console.log('searchTermTrimmed', searchTermTrimmed); // map the search hits to the search pages
...
function vfShowMore(scope) {
  ...
  targetList.forEach(function (targetContent) {
    var itemsToShow = Math.round(targetContent.dataset.vfJsShowMorePagerSize); // generate the appropriate style to show
  ...
  var showIt = function showIt(targetContent) {
    ...
    targetContent.focus(); // Make the active tab focusable by the user (Tab key)
  ...
  Array.prototype.forEach.call(buttonList, function (button, i) {
    button.parentNode.setAttribute('role', 'presentation'); // Reset any active tabs from a previous JS call
...
function vfLocationNearestDomActions(scope) {
  var scope = scope || document; // Get items from dom
